...
static int getcurrent(void)
{
    ...
    /* カレント・スレッドは必ず先頭にあるはずなので先頭から抜き出す */
...
{
    /*
     * レディー・キューからいったん外して接続しなおすことで
     * ラウンド・ロビンで他のスレッドを動作させる
     */
    putcurrent();
    return 0;
}
...
{
    /*
     * レディー・キューから外されたままになるのでスケジューリングされなくなる
     */
    return 0;
}
...
static void schedule(void)
{
    ...
    /*
     * 優先度の高い順(優先度の数値の小さい順)にレディー・キューを見て
     * 動作可能なスレッドを検索する
     */
...
static void thread_intr(softvec_type_t type, unsigned long sp)
{
    ...
    /*
     * スレッドのディスパッチ
     * (dispatch() 関数の本体は startup.s にありアセンブラで記述されている)
     */
